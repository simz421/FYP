...
settings_thresholds_bp = Blueprint("settings_thresholds", __name__)
...
def _normalize_parameter(p: Any) -> str:
    """
    Ensures parameter is a non-empty string.
    Raises ValueError if invalid (examiner-safe).
    """
    if p is None:
...
@settings_thresholds_bp.put("/settings/thresholds")
def put_threshold():
    """
    ...
    try:
        ...
        data = upsert_threshold_rule(
            device_id=device_id,
            node_id=node_id,
            parameter=parameter,     # ✅ always str now
            min_value=min_value,
            max_value=max_value,
...
@settings_thresholds_bp.get("/settings/thresholds")
def get_thresholds():
    """
