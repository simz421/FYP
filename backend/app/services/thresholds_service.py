    ...
    *,
    ...
    for p in parameters:
        rule = (
            ThresholdRule.query
            .filter(
                ThresholdRule.parameter == p,
                ThresholdRule.is_enabled.is_(True),
            )
            .filter(
                (ThresholdRule.node_id == node_id)
                | ((node_id is None) & (ThresholdRule.node_id.is_(None)))
            )
            .filter(
                (ThresholdRule.device_id == device_id)
                | ((device_id is None) & (ThresholdRule.device_id.is_(None)))
            )
            .order_by(
                ThresholdRule.node_id.desc(),
                ThresholdRule.device_id.desc(),
            )
            .first()
        )
        ...
        if rule:
            scope = (
                "node" if rule.node_id is not None
                else "device" if rule.device_id is not None
                else "global"
            )
