...
def list_alerts(
    *,
    ...
    if parameter:
        q = q.filter(AlertEvent.parameter == parameter.strip().lower())
    if active_only:
        q = q.filter(AlertEvent.is_active == True)  # noqa: E712
    if acked_only is True:
        q = q.filter(AlertEvent.is_acked == True)  # noqa: E712
    if acked_only is False:
        q = q.filter(AlertEvent.is_acked == False)  # noqa: E712

    q = q.order_by(AlertEvent.created_at.desc()).limit(max(1, min(int(limit), 1000)))
